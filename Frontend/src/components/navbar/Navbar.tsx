...
const CourseEnrollmentModal = lazy(() =>
  import("@/components/ui/CourseEnrollmentModal").then((module) => ({
    default: module.CourseEnrollmentModal,
  })),
);
...
export const Header = () => {
  ...
  const { isAuthModalOpen, authModalMode, openAuthModal, closeAuthModal } =
    useUIStore();

  // Helper getters/setters to maintain compatibility with existing components
  const setShowAuthModal = (show: boolean) =>
    show ? openAuthModal(authModalMode) : closeAuthModal();
  ...
  return (
    <>
      ...
      >
        ...
        {/* Main Navbar */}
        <nav
          className={`bg-background/95  py-2 backdrop-blur-md border-b border-border/40 transition-all duration-500 flex items-center ${isScrolled ? "h-18 md:h-20" : "h-20 md:h-24"}`}
        >
          <div className="container mx-auto px-4 sm:px-6 flex items-center justify-between h-full">
            ...
            <Link to="/" className="flex-shrink-0">
              <img
                src={logo}
                alt="AOTMS"
                className={`w-auto transition-all duration-300 hover:scale-105 ${isScrolled ? "h-10 md:h-12" : "h-16 md:h-20"}`}
              />
            </Link>
            ...
            <div className="hidden lg:flex items-center gap-8 xl:gap-10">
              {navLinks.map((link) => (
                ...
                >
                  <Link
                    to={link.href}
                    className={`flex items-center gap-1.5 text-base font-semibold transition-colors duration-200 ${
                    ...
                    <AnimatePresence>
                      {activeDropdown === link.name && (
                        <motion.div
                          ...
                          className={`absolute top-[calc(100%+12px)] left-1/2 -translate-x-1/2 bg-popover rounded-xl shadow-xl border border-border/50 overflow-hidden z-[150] ${
                            (link as NavLink).menuCategories
                              ? "w-[700px] p-4"
                              : link.isMegaMenu
                                ? "w-[600px] -left-20 translate-x-[-20%] p-2"
                                : "w-64 p-2"
                          }`}
                        >
                          ...
                          {(link as NavLink).menuCategories ? (
                            <div className="grid grid-cols-3 gap-y-4 gap-x-6 relative z-10">
                              {(link as NavLink).menuCategories!.map(
                                (category: MenuCategory, idx: number) => (
                                      ...
                                      {category.courses.map(
                                        (course: Course, cIdx: number) => {
                                          ...
                                          return (
                                            ...
                                            >
                                              <div
                                                className={`p-1.5 rounded-md transition-colors ${isActive ? "bg-primary text-white" : "bg-primary/10 text-primary group-hover/item:bg-primary group-hover/item:text-white"}`}
                                              >
                                                <course.icon className="w-3.5 h-3.5" />
                                              </div>
                                              <span
                                                className={`text-sm font-bold transition-colors ${isActive ? "text-primary" : "text-slate-900 group-hover/item:text-primary"}`}
                                              >
                                                {course.name}
                                              </span>
                                            </Link>
                            ...
                            <div
                              className={`grid gap-1 relative z-10 ${link.isMegaMenu ? "grid-cols-2 p-2" : "grid-cols-1"}`}
                            >
                              {link.dropdownItems?.map((item) => {
                                ...
                                return (
                                  ...
                                  >
                                    <div
                                      className={`p-2 rounded-md transition-colors ${isActive ? "bg-[#0066CC] text-white" : "bg-primary/10 text-primary group-hover/item:bg-[#0066CC] group-hover/item:text-white"}`}
                                    >
                                      {item.icon && (
                                    ...
                                    <div>
                                      <div
                                        className={`text-sm font-bold transition-colors ${isActive ? "text-[#0066CC]" : "text-foreground group-hover/item:text-[#0066CC]"}`}
                                      >
                                        {item.name}
                                      </div>
              ...
              {isLoggedIn ? (
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-[calc(100%+12px)] w-64 bg-white rounded-xl shadow-2xl border border-slate-100 p-2 invisible opacity-0 group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-[150]">
                    ...
                    <Link
                      to="/dashboard"
                      className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg transition-colors mb-1"
                    >
                    ...
                    <button
                      onClick={logout}
                      className="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium text-[#DC2626] hover:bg-red-50 rounded-lg transition-colors"
                    >
                ...
                <div className="flex items-center gap-3">
                  <Button
                    ...
                    className="h-10 px-6 font-bold rounded-full shadow-lg bg-[#0066CC] shadow-primary/20 hover:shadow-primary/30 hover:scale-105 active:scale-95 transition-all"
                  >
                    Book Free Demo
                  </Button>
                </div>
